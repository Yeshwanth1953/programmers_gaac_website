...
export default function TeamLayout({
  ...
  vision,
  activities,
  teamKey, // 🔥 IMPORTANT: unique key like "stargazers"
}) {
    ...
    const fetchMembers = async () => {
      try {
        ...
        const res = await fetch(
          `https://your-backend-url.com/api/teams/${teamKey}/members`
        );
  ...
  const leads = members.filter((m) => m.role?.toLowerCase().includes("president") || m.isLead);
    ...
    <div className="min-h-screen bg-[#020a12] text-white relative overflow-hidden">
      ...
      <section className="max-w-7xl mx-auto px-6 pb-24">
        <div className="grid md:grid-cols-2 gap-12">
          {[{ label: "Mission", text: mission }, { label: "Vision", text: vision }].map(
            (item, i) => (
              <div
                key={i}
